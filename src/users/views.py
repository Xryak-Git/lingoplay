...
async def update_user(
    user_update_in: UserUpdate, current_user: CurrentUser, user_service: Annotated[UsersService, Depends(user_service)]
) -> UserRead:
    """Update user."""
    try:
        user = await user_service.update(id=current_user.id, fields=user_update_in)
    except UserAlreadyExistsError as e:
        raise HTTPException(
            status_code=409,
            detail=[{"msg": f"{e.field} '{e.value}' already exists."}],
